...
func TestAuditRunConfigJoinShowsApply(t *testing.T) {
	...
	if code := cmdRun([]string{"software-engineer", "fix-bug", "--input", "x", "--as", "dana@example.com", "--config", root, "--log-dir", logs, "--executor", "echo", "--artifact-dir", t.TempDir(), "--workspace", t.TempDir()}, &out); code != 0 {
		t.Fatalf("run: %d\n%s", code, out.String())
	}
