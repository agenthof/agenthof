...
func (f *fakeExecutor) Execute(ctx context.Context, a config.AgentDef, input string, artifacts map[string]string) (engine.StepResult, error) {
	f.called = true
...
func TestMuxExecutor_Execute_Contained(t *testing.T) {
	...
	res, err := mux.Execute(context.Background(), agentDef, "in", nil)
	if err != nil {
...
func TestMuxExecutor_Execute_UnknownTier(t *testing.T) {
	...
	_, err := mux.Execute(context.Background(), agentDef, "in", nil)
	if err == nil {
